...
# amended to bosh2
echo "Adding blobs"
bosh add-blob "${go_pkg_path}" golang/go-linux-amd64.tar.gz
bosh add-blob "${go_version_path}" golang/go-version.txt
bosh add-blob "${splunk_pkg_path}" splunk/splunk-linux-x86_64.tgz
bosh add-blob "${splunk_version_path}" splunk/splunk-version.txt

#add3 - linked to add1 and add2
bosh add-blob "${common_pkg_path}" common/splunk-linux-x86_64.tgz
# end add3

# amended to bosh2
echo "Creating release"
create_cmd="bosh create-release --name cf-splunk --tarball="./release.tgz" --force"
if [ "$version" != "" ]; then
